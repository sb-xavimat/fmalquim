...
# - La primera part indica l'idioma (ca, es).
# - La segona part indica el tipus de substància (1, 2, 3, 5, 7, 8, 10).
# - La tercera part indica el sistema de nomenclatura (1, 2, 3, 4).
# - La quarta part indica el mode de pregunta (1, 2).
# Actualment:
# - Els `namesys` són numèrics.
# - Els `mode` són numèrics.
# Volem que:
# - Els `namesys` siguin text (1=PRE, 2=NOX, 3=TRA, 4=SIS).
...
def transform_help():
    ...
    with open("helptransform.json", "w", encoding="utf-8") as f:
        json.dump(new_help, f, indent=4, ensure_ascii=False)
