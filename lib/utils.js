...
// Utils
// MARK: imports
import { PREFIXES, ROMANS } from "./constants.js";
...
/**
 * Obtindre el prefix segons el subíndex i la llengua.
 * @param {string} lang
 * @param {string} sub
 * @returns {string}
 */
function getPrefix(lang, sub) {
  let prefix = PREFIXES[sub];
  if (prefix && lang === "eu") {
    // En euskera, la "c" canvia a "k"
    prefix = prefix.replace("c", "k");
  }
  return prefix || "";
}
