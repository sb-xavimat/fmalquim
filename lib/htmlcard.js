...
function createStyledSpan(item) {
  ...
  span.style.color = item.color || '#000000'; // Default color if not found
...
function createHTMLCard(helpData, debug = false) {
  ...
  for (let i = 1; i < cardData.lines.length; i++) {
    ...
    if (rightText && rightText.includes("_")) {
      ...
      rightText = rightText
        .replace(/_‑uro_/g,
          `<span style='color:${COLOR.lilac}'><i>‑uro</i></span>`
        )
        .replace(/_‑ur_/g,
          `<span style='color:${COLOR.lilac}'><i>‑ur</i></span>`
        )
        .replace(/_‑ide_/g,
          `<span style='color:${COLOR.lilac}'><i>‑ide</i></span>`
        );
